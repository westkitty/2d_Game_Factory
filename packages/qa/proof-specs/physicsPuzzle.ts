...
export async function run(harness: Harness): Promise<SmokeOutcome> {
  ...
  const startedOk = booted.installedPacks.includes('sw2d.puzzle') && initial.puzzle?.kind === 'physics-goal' && initial.puzzle.solved === false && (initial.puzzle.ball?.x ?? 999) < 400;
  ...
  // One nudge sends it across; the code-seam puzzle (sw2d.puzzle configSource 'code') marks it solved in the goal.
