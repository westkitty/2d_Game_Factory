...
export async function run(harness: Harness): Promise<SmokeOutcome> {
  ...
  const startedOk =
    ...
    booted.installedPacks.includes('sw2d.items') &&
  ...
  const picked = await holdUntil(harness, ['ArrowUp'], read, (s) => s.kartItem?.held === true, 80, 3);
  evidence.picked = picked.kartItem;
  const pickOk = picked.kartItem?.held === true;
  await harness.keyTap('KeyJ');
  await harness.stepFrames(6);
  const boosted = await read();
  evidence.boosted = { kart: boosted.kartItem, drive: boosted.drive };
  const boostOk = boosted.kartItem?.lastResult === 'fired' && boosted.kartItem.fired >= 1 && !boosted.kartItem.held;

  const done = await holdUntil(harness, ['ArrowUp'], read, (s) => s.drive?.outcome === 'complete');
  evidence.done = done.drive;
  const doneOk = done.drive?.outcome === 'complete' && done.drive.lastResult === 'distance' && done.drive.score >= 80 && done.drive.best >= 80;

  const run = await restartRun(harness);
  const fresh = await read();
  evidence.restart = { ...run, drive: fresh.drive, kart: fresh.kartItem };
  const restartOk = run.after === run.before + 1 && fresh.drive?.outcome === 'playing' && fresh.drive.score < 80 && fresh.drive.best >= 80 && !fresh.kartItem?.held;

  await harness.keyDown('ArrowUp');
  await harness.keyDown('ArrowLeft');
  const crashed = await holdUntil(harness, ['ArrowUp', 'ArrowLeft'], read, (s) => s.drive?.outcome === 'failed', 120, 3);
  await harness.keyUp('ArrowUp');
  await harness.keyUp('ArrowLeft');
  evidence.crashed = crashed.drive;
  const crashOk = crashed.drive?.outcome === 'failed' && crashed.drive.lastResult === 'crash';

  const passed = startedOk && idleOk && movingOk && pauseOk && pickOk && boostOk && doneOk && restartOk && crashOk;
  return { passed, details: { ...evidence, startedOk, idleOk, movingOk, pauseOk, pickOk, boostOk, doneOk, restartOk, crashOk } };
}
