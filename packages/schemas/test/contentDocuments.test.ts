...
describe('validateContentBundleData', () => {
  ...
  it('knows which schema is expected before validating anything', () => {
    expect(Object.keys(CONTENT_DOCUMENTS)).toEqual(['tuning', 'items', 'weapons', 'encounters', 'puzzles', 'generation', 'world-graph', 'vehicles', 'races', 'economy', 'needs', 'dialogue', 'perception', 'ball-paddle', 'melee', 'local-play', 'stage-scroll', 'timing', 'wall', 'territory', 'pinball', 'camera', 'codex', 'targeting', 'pursuit', 'runs', 'simulation']);
  });
