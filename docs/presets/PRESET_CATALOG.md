...
Phase 7A: 27 registered composition recipes across three families. A recipe is a
composition of real controller families and real `@sw2d/packs` system packs - never an
engine fork (MASTER_PROJECT.md section 3.1). Every recipe below is `maturity: "recipe"`:
none has a functional smoke demo (Phase 8) or a deep end-to-end proof (Phase 10) yet.
...
match) - if the catalog changes, update this file in the same change.
