...
| `npm test` | 1787 unit tests pass: pure logic, schema validation, generator determinism, capability/pack composition, resource governance, checksum/notices mechanics, code-dependency provenance completeness. | Anything that needs a real browser (canvas rendering, real DOM layout, real input events). |
...
| `npm run qa:smoke` | 14 targets (12 demo games + 2 starter pages) each boot, enter play with every declared system pack installed, and complete a scripted real-player-shaped interaction - through real system Chrome via `playwright-core`, against a real production build, with zero console errors and zero external requests. | Any preset not one of these 14 (the other 60 stay `recipe`/`smoke-validated`-by-family-equivalence only via `npm run qa:matrix`, not by a hand-written spec). Real wall-clock performance (see "Deterministic frame stepping" below). |
...
`qa:proof`, `qa:responsive`, `qa:matrix` and `release:verify` are not part of `validate` because each builds
