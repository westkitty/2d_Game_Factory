 ...
 * `npm run sw2d -- add-theme proof-sokoban <theme-id>`. Every generated game
 ...
 * add more with `npm run sw2d -- add-level proof-sokoban <level-id>`.
 */
...
export const gameContent: ContentSource = {
  ...
  load: async (): Promise<ContentBundle> => {
    ...
    const data = validateContentBundleData({ tuning: tuningData, 'levels/main': normalizedLevel, puzzles: puzzlesData });
