...
## Phase 13 Extension: Dungeon Chests, Lockpicking & Rarity Drops

Exercised capabilities:
- `sw2d.dungeon-chests` (`ChestsService`, `LockpickingService`), providing `loot.chests` and `loot.lockpicking`.
- `sw2d.items` (`ItemsService`), providing `items.state`.

Extended Journey & Oracles:
1. All Phase 7 generation and room-graph requirements remain verified and intact.
2. Generated dungeon rooms contain four deterministic chests:
   - Wooden unlocked chest (`chest-wooden`)
   - Silver key-locked chest (`chest-silver`)
   - Gold pick-locked chest (`chest-gold`)
   - Mimic trap chest (`chest-trap`)
3. Wooden chest opens and drops common loot; repeated opens return typed `'already_open'` and never duplicate loot.
4. Silver chest opening fails with typed `'locked_needs_key'` without key; once silver key is granted via `ItemsService`, opening succeeds, consumes key, and grants silver drops.
5. Gold chest lockpicking session demonstrates pick torque damage on incorrect angles and completes upon reaching tolerance; chest unlocks and grants rare/epic drops.
6. Mimic trap chest opening triggers typed trap activation and emits `loot:trapTriggered`.
7. Determinism: Same seed produces identical room layout, chest placement, sweet spots, and loot rolls across instantiations.
