 ...
 * `npm run sw2d -- add-theme proof-twin-stick-shooter <theme-id>`. Every generated game
 ...
 * add more with `npm run sw2d -- add-level proof-twin-stick-shooter <level-id>`.
 */
...
export const gameContent: ContentSource = {
  ...
  load: async (): Promise<ContentBundle> => {
    ...
    const data = validateContentBundleData({ tuning: tuningData, 'levels/main': normalizedLevel, weapons: weaponsData });
