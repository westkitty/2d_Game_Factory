 ...
 * `npm run sw2d -- add-theme proof-run-and-gun <theme-id>`. Every generated game
 ...
 * add more with `npm run sw2d -- add-level proof-run-and-gun <level-id>`.
 */
...
export const gameContent: ContentSource = {
  ...
  load: async (): Promise<ContentBundle> => {
    ...
    const data = validateContentBundleData({ tuning: tuningData, 'levels/main': normalizedLevel, items: itemsData, weapons: weaponsData });
