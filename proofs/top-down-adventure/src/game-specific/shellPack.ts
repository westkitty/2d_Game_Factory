import type { InstalledSystemPack, ItemsService, NormalizedLevel } from '@sw2d/contracts';
import { bindCollectiblePickups, topDownController, type SceneContext, type ScenePackDefinition } from '@sw2d/runtime';
import { CAPABILITY_IDS, type ProgressionService, type WorldService } from '@sw2d/packs';
...
export const GAME_SPECIFIC_PACK: ScenePackDefinition = {
  ...
  install(context: SceneContext): InstalledSystemPack {
    ...
    const debugHandle = context.debug.contribute('game.top-down-shell', () => ({
      ...
      // Item effects land in the real reusable services (world flags,
      // progression currency/xp) - read back here, not shell-local.
      hasMapKey: world.hasFlag('hasMapKey'),
      currency: progression.currency(),
      xp: progression.xp(),
      canEatRation: items.canConsume('ration'),
    }));
    ...
    return {
      ...
      update(): void {
        ...
        // Press INTERACT to eat a ration: removes one unit and applies its
        // effect (progression.xp) through the reusable service.
        if (intent.interactPressed && items.canConsume('ration')) {
          items.consume('ration', 1);
        }
      },
