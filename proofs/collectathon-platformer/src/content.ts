 ...
 * `npm run sw2d -- add-theme proof-collectathon-platformer <theme-id>`. Every generated game
 ...
 * add more with `npm run sw2d -- add-level proof-collectathon-platformer <level-id>`.
 */
...
export const gameContent: ContentSource = {
  ...
  load: async (): Promise<ContentBundle> => {
    ...
    const data = validateContentBundleData({ tuning: tuningData, 'levels/main': normalizedLevel, items: itemsData });
