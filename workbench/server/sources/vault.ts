...
export function vaultStore(input: VaultStoreInput): VaultPackRecord {
  ...
  const record: VaultPackRecord = {
    ...
    acquiredAt: existing?.acquiredAt ?? iso,
    lastVerifiedAt: iso,
  };
...
/**
 * Re-checks a stored record's recorded licence against the *current* policy and
 * stamps a fresh verification time. Does not re-fetch the provider - the licence
 * snapshot is the governing evidence; this catches the case where a licence has
 * since been removed from the accepted list.
 */
export function reverifyVault(sha256: string, now: number = Date.now()): VaultEntryView {
  ...
  const updated: VaultPackRecord = { ...record, rights: re, lastVerifiedAt: new Date(now).toISOString() };
  writeIndex(packs.map((entry) => (entry.sha256 === sha256 ? updated : entry)));
  return { ...updated, freshness: re.status, bytesPresent: existsSync(bytesPathFor(sha256)) };
}
