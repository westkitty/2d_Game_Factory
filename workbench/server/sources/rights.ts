...
/** Whether a pack in this rights state may be acquired/used at all. Unsupported and unknown are hard blocks. */
export function rightsAllowUse(rights: RightsEvidence): boolean {
  return rights.status === 'verified' || rights.status === 'attribution-required' || rights.status === 'stale-verification';
}
